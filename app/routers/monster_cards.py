...
def create_monster_card(payload: MonsterCardCreate, db: DbSession):
    try:
        return MonsterCard.create_card(db, payload)
...
def get_monster_card_by_name(name: str, db: DbSession):
    card = MonsterCard.get_card_by_name(db, name)
    if not card:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Card not found.")
    return card
...
def patch_monster_card(card_id: int, payload: MonsterCardUpdate, db: DbSession):
    ...
    try:
        return MonsterCard.update_card(db, card_id, updates)
...
def remove_monster_card(card_id: int, db: DbSession):
    try:
        MonsterCard.delete_card(db, card_id)
...
def render_monster_card(card_id: int, db: DbSession):
    try:
        return MonsterCard.display_monster_card(db,card_id)
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
